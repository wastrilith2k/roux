...
companion-framework/
  src/
    core/              # Conversation pipeline, internal state, memory retrieval
    tasks/             # 34 Celery background tasks (fact extraction, episodes, etc.)
    autonomy/          # Proactive reach-out, goals, opinions, reflection
    memory/            # 24 memory modules (facts, episodes, semantic, graph)
    integrations/      # Google (Gmail + Calendar), Cloudinary
    scheduling/        # Calendar-aware scheduling
    llm/               # LLM provider abstraction (OpenRouter, Fireworks, OpenAI, Anthropic)
    routes/            # Flask endpoints (chat, observe, integrations, auth)
    handlers/          # Message entry point
    voice/             # TTS integration (Deepgram, ElevenLabs, Edge TTS)
  instances/           # Companion definitions (personality, profiles, life events)
  ...
  tests/               # 105 passing tests (13 test files)
